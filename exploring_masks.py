...
def read_image_labels(image_id, cyc):
...
def plot_images_masks(image_ids, cyc):
    plt.close('all')
    fig, ax = plt.subplots(nrows=len(image_ids)//2,ncols=2, figsize=(64,64))
    for ax_index, image_id in enumerate(image_ids):
        image = read_image_labels(image_id, cyc)
        ax[ax_index//2, ax_index%2].imshow(image)
    if cyc > 0:
        if cyc > 1:            
            if cyc > 2:
...
plot_images_masks(random_image_ids, 0)
plot_images_masks(random_image_ids, 1)
plot_images_masks(random_image_ids, 2)
plot_images_masks(random_image_ids, 3)
